...
export default {
  ...
  execute: async (message: Message, args: Array<string>, client: Client) => {
    if (!message.guild.me.hasPermission("MANAGE_ROLES"))
      return await message.channel.send("I don't have permission to use this command!");
    ...
    if (!member)
      return await message.channel.send("You must specify a valid of this server!");
    ...
    if (member.roles.cache.has(muterole.id))
      await member.roles.remove(muterole.id).then(async () => {
        await message.channel.send(`<@${member.user.id}> has been unmuted by <@${message.author.id}>!`);
      });
  },
};
