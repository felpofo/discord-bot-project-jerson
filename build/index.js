...
if (!config.webhook.enabled) {
    ...
    u.warn("Webhook enabled".yellow.underline);
if (process.argv[2]) {
    if (process.argv[3])
...
for (const file of commandFiles) {
    import(`./commands/${file}`)
        ...
        if (config.debug)
            log(`[ ${"DEBUG".cyan.bold} ] ` + `Loaded command: ${mod.default.magenta.underline}`);
    })
        .catch((err) => {
...
if (config.webhook.enabled) {
    ...
    if (webhook.id)
        u.info("Webhook Initializated".yellow.underline);
}
client.once("ready", () => {
...
client.on("message", (message) => {
    if (message.channel.type == "text") {
        if (message.embeds[0]) {
            ...
            if (embed.title)
                log(`[ ${message.author.tag} ] [ ${"TITLE".cyan.bold} ] ${embed.title.replace(regex, "")}`);
            if (embed.description)
                log(`[ ${message.author.tag} ] [ ${"DESC".cyan.bold} ] ${embed.description.replace(regex, "")}`);
            if (embed.footer)
                log(`[ ${message.author.tag} ] [ ${"FOOTER".cyan.bold} ] ${embed.footer.text.replace(regex, "")}`);
        }
        else {
    ...
    if (message.channel.type == "dm") {
        ...
        if (message.content.match(/(sou)\s*(foda)\s*\w+/g)) {
            ...
            message.channel.send(new MessageAttachment(video, "SPOILER_sex.mp4"));
        }
    }
    if (message.author.bot)
        return;
...
const rl = readline.createInterface({
    ...
    completer: (line) => {
        ...
        formatted_commands.forEach((value, index) => (formatted_commands[index] = value.replace("_", " ")));
...
rl.commands = {
    ...
    set_category: (args) => u.set_category(String(args[0]), String(args[1])),
    get: () => u.get(),
    get_chat: () => u.get_chat(),
