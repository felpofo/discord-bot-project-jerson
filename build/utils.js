...
export function log_message(channel, user, message) {
    if (channel.type == "dm")
        log(`[ ${"DM".cyan.bold} ] [ ${user.username} ] ${message.content}`);
    else
        log(`[ ${channel.name.magenta.bold} ] ${user.username.underline}: ${message.content}`);
}
export function update_chat(id, log = true) {
    config.channel.id = undefined;
    config.channel.name = undefined;
    if (!id)
        return warn("ID inválido");
    if (id.length != 18)
        return warn("ID inválido");
...
export function send_bot_message(message) {
    if (message == "")
        return warn("Mensagem vazia");
    if (!config.channel.id)
        return warn("Chat não definido");
    try {
...
export function set_chat(id) { throw new Error("Not implemented"); }
export function set_category(id, permission) { throw new Error("Not implemented"); }
export function get() {
    log("Comandos:".bold);
...
export function exit(exit_code) {
